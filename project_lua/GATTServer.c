...
vm_bt_gatt_connection_t connection; 
VM_BT_GATT_CHARACTERISTIC_HANDLE attribute_handle;
...
void app_init(void)
{
    vm_log_debug("[TestApp] app_init_begain g_app_cntx.state= %d g_app_cntx.op_flag = %d\n",g_app_cntx.state, g_app_cntx.op_flag);
...
void app_deinit(void)
{
    ...
    connection.context_handle = NULL;
...
void app_connection_cb(const vm_bt_gatt_connection_t *conn, VMBOOL connected, const vm_bt_gatt_address_t *bd_addr)
{
    ...
    if(connected)
    {
        ...
        connection.connection_handle = conn->connection_handle;
        connection.context_handle = conn->context_handle;
        vm_log_debug("[TestApp] connect success, and not find in list!\n");
        vm_log_debug("%x,%x\n",connection.connection_handle,connection.context_handle);
    }
    return;
}
        ...
        if(g_app_cntx.state == APP_STATUS_ENABLING)
        {
            if(status == 0)
            {
                if((g_app_cntx.op_flag == APP_OP_ADD_CHARACTERISTIC) && (g_srvc_handle == srvc_handle))
                {
                    ...
                    vm_log_debug("[TestApp] app_add_start_service %x! %x!\n", srvc_handle, char_handle);
...
// todo notify data to client
VM_RESULT app_send_indication(
        VMBOOL need_confirm, vm_bt_gatt_attribute_value_t* value)
{
    if(connection.connection_handle == NULL
    && connection.context_handle == NULL){
        return VM_FAIL;
    }
    
    return vm_bt_gatt_server_send_indication(&connection, attribute_handle, need_confirm, value);
}
                                      ...
                                      VM_BT_GATT_ATTRIBUTE_HANDLE attr_handle, VMUINT16 offset, VMBOOL is_long)
{
    ...
    if(g_char_handle == attr_handle)
    {
        ...
        else if(uuid == 0x2AA0)
        {
            ...
            vm_log_debug("[TestApp] app_request_read_callback_success,%x,%x attr_handle %x, uuid %x!\n",conn->connection_handle,conn->context_handle, attr_handle, uuid);
            return;
        }
        read_att_value.length = 1;
